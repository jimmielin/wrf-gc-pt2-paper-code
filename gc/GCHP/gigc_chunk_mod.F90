...
!
    ...
    REAL(hp)                          :: Ap(State_Grid%NZ+1), Bp(State_Grid%NZ+1)
    INTEGER                           :: L
    INTEGER                           :: IM, JM, LM
    ...
    ! Stubbed out for WRF-GC 2.1 - two-domain work will follow
    ! in the future
    ...
    ! (Allocate State_Grid%. This will be re-performed by Switch_Dims later)
...
!
    ...
    call GIGC_State_Get_Opt( Input_Opt%AmIRoot, Input_Opt )

    ! Get the Diagnostics List from GIGC_State_Boot
    call GIGC_State_Get_DiagList( Input_Opt%AmIRoot, DiagList )
    ...
    call GIGC_Switch_Dims( Input_Opt%AmIRoot, ID,            &
                           lonCtr, latCtr, lonEdge, latEdge, &
    ...
    ! Set grid based on passed mid-points and edges (WRF)
    CALL SetGridFromCtrEdges( Input_Opt%AmIRoot, State_Grid, lonCtr, latCtr, lonEdge, latEdge, RC )
    ...
    CALL SET_TIMESTEPS( am_I_Root  = Input_Opt%AmIRoot,                          &
    ...
    CALL GC_Init_StateObj( Input_Opt%AmIRoot, DiagList, Input_Opt, &
                           State_Chm, State_Diag, State_Grid, State_Met, RC )

    ! Initialize other GEOS-Chem modules
    CALL GC_Init_Extra( Input_Opt%AmIRoot, DiagList, Input_Opt,    &
...
# if defined( MODEL_GEOS )
    ...
    IF ( Input_Opt%LUCX ) THEN
       ...
       CALL INIT_UCX( Input_Opt%AmIRoot, Input_Opt, State_Chm, State_Diag, State_Grid )
    ...
    !=======================================================================
    CALL GIGC_State_Set_Opt( Input_Opt%AmIRoot, Input_Opt, HcoConfig)
    CALL GIGC_State_Init(am_I_Root    = Input_Opt%AmIRoot,  &
                         ID           = ID,                 &
                         State_Met    = State_Met,          &
                         State_Chm    = State_Chm,          &
...
!BOC
    ...
    IF ( DoConv .OR. DoChem .OR. DoWetDep ) THEN
       CALL SETUP_WETSCAV( Input_Opt%AmIRoot,  Input_Opt, State_Chm, &
                           State_Grid, State_Met, RC )
    ENDIF
    ...
    CALL Compute_Olson_Landmap( Input_Opt%AmIRoot, Input_Opt, State_Grid, &
                                State_Met, RC )
    ...
    ! MODISLAI is transferred from grid%lai in WRF-GC
!#if !defined( MODEL_WRF ) 
...
!#endif
    ...
    CALL Accept_External_Pedge    ( am_I_Root      = Input_Opt%AmIRoot,  &
                                    State_Met      = State_Met,  &
                                    RC             = RC         )
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    ! WRITE(6, *) "O2:", State_Chm%Species(1, 1, 1, IND_('O2'))
    ...
    WRITE(6, *) "HNO3:", State_Chm%Species(1, 1, 1, IND_('HNO3'))
    ...
    WRITE(6, *) "HNO3:", SUM(State_Chm%Species(:, :, 1, IND_('HNO3')))
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    IF ( DoChem ) THEN
       ...
       IF ( IND_('H2O','A') > 0 ) THEN
          CALL SET_H2O_TRAC( Input_Opt%AmIRoot, (.not. Input_Opt%LUCX), Input_Opt, &
                             State_Chm, State_Grid, State_Met, RC )
       ENDIF
